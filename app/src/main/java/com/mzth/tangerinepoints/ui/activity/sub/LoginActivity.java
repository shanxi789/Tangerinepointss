...
//        String UUID = Constans.GetUUID(_context);
//        map.put("app_instance_id",UUID);
        String uuid = (String) SharedPreferencesUtil.getParam(_context, SharedName.UUID,"");
        map.put("app_instance_id", uuid);
    ...
    private void MessagePushRequest(String token){
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("token",token);
        NetUtil.Request(NetUtil.RequestMethod.POST, Constans.MESSAGE_PUSH, map, Authorization,MainApplication.APP_INSTANCE_ID, new NetUtil.RequestCallBack() {
            @Override
            public void onSuccess(int statusCode, String json) {
                ToastUtil.showShort(_context,json);
            }
