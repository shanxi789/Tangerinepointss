...
public class GiftFragment extends BaseFragment implements
    ...
    private Location location2;//GPS 与 网络定位
    ...
    protected void initData() {
        ...
        if (permissionCheck != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions
                    (getActivity(), new String[]{Manifest.permission.
                                    ACCESS_FINE_LOCATION},
                            1);
        }
        String accesskey = (String) SharedPreferencesUtil.getParam(_context, SharedName.AccessKey, "");
        ...
        //判断本地是否有offer信息 如果有取本地的
        String offers = (String) SharedPreferencesUtil.getParam(_context, SharedName.OFFER, "");
        if (StringUtil.isEmpty(offers)) {
            //默认类型
            OffersRequest();
        } else {
...
//        String xi = southwest.latitude+","+southwest.longitude;
        ...
        NetUtil.Request(NetUtil.RequestMethod.GET, Constans.BUSINESSES, map, Authorization, MainApplication.APP_INSTANCE_ID, new NetUtil.RequestCallBack() {
            @Override
    ...
    private void OffersRequest() {
        ...
        map.put("n", n);
        //获取当前位置
        String locations = (String) SharedPreferencesUtil.getParam(_context, SharedName.LOCATION,"");
        //map.put("location", locations);
        map.put("location", Constans.location);
        NetUtil.Request(NetUtil.RequestMethod.GET, Constans.OFFERS, map, Authorization, MainApplication.APP_INSTANCE_ID, new NetUtil.RequestCallBack() {
            @Override
            public void onSuccess(int statusCode, String json) {
                //ToastUtil.showShort(_context,json);
                //有网时隐藏
                //MainActivity.tv_Off.setVisibility(View.GONE);
                MainActivity.ll_inter_no.setVisibility(View.GONE);
                String offers = GsonUtil.getJsonFromKey(json, "offers");
                //保存offer
                SharedPreferencesUtil.setParam(_context, SharedName.OFFER, offers);
                list = GsonUtil.getListFromJson(offers, new TypeToken<List<OffersBean>>() {
                });
                location = new ArrayList<Double>();
                maps = new HashMap<Double, Double>();
...
//                        getActivity().finish();
            ...
            public void onFailure(Exception e, String errorMsg) {
                //没网时显示
                //MainActivity.tv_Off.setVisibility(View.VISIBLE);
                MainActivity.ll_inter_no.setVisibility(View.VISIBLE);
                ToastUtil.showShort(_context, errorMsg);
                //结束刷新加载
                finishRefresh();
                WeiboDialogUtils.closeDialog(mWeiboDialog);//请求成功关闭动画
...
//        if(mGoogleApiClient.isConnected() && location2 != null){
//            startIntentService(new LatLng(location2.getLatitude(),location2.getLongitude()));
//        }
