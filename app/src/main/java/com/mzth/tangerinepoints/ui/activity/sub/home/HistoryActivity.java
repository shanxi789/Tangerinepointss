...
public class HistoryActivity extends BaseBussActivity {
    ...
    protected void initData() {
        super.initData();
        //将用户登录的日期保存到本地
        long time = (long) SharedPreferencesUtil.getParam(_context, SharedName.LOGIN_TIME,0L);
        if(!StringUtil.isEmpty(time)) {
            String sinceDate = DateFormat.getDateInstance
                    (DateFormat.MEDIUM, Locale.US).format(new Date(time));
            login_time.setText("total points you have earned since "+sinceDate);
        }
        //设置标题
    ...
    private void HistoryRequest(){
        ...
        NetUtil.Request(NetUtil.RequestMethod.GET, Constans.TRANSACTIONS, map, Authorization, MainApplication.APP_INSTANCE_ID, new NetUtil.RequestCallBack() {
            @Override
